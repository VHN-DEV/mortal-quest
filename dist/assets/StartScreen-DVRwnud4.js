import{X as r,A as u,s as i}from"./index-OX4Z4gcz.js";import"./vendor-capacitor-DNzPfH2b.js";import"./vendor-gsap-CzGW6FVa.js";class w{constructor(){this.container=document.getElementById("screen-start"),this.init()}init(){this.container||(this.container=document.createElement("div"),this.container.id="screen-start",this.container.className="screen flex flex-col h-full z-[200] overflow-hidden",document.querySelector("main").appendChild(this.container))}async render(){const n=await r.getAllMetadata();let e=1;Object.values(n).forEach(s=>{s.realmId>e&&(e=s.realmId)});let t="bg-start-default";e>40?t="bg-start-immortal":e>20&&(t="bg-start-expert"),this.container.innerHTML=`
        ...
        `,this.bindEvents()}bindEvents(){const n=document.getElementById("btn-start-new");n&&(n.onclick=async()=>{const l=await r.getAllMetadata();let o=null;for(let a=1;a<=5;a++)if(!l[a]){o=a;break}o?(r.currentSlot=o,window.game.showCreationScreen()):(i.ui.toast("Toàn bộ Mệnh Đồ Lục đã đầy. Hãy chọn một ô để ghi đè hoặc xóa bớt.","warning"),await window.game.screens.save.render(),i.ui.switchScreen("screen-save"))});const e=document.getElementById("btn-continue-game");e&&(e.onclick=async()=>{await window.game.screens.save.render(),i.ui.switchScreen("screen-save")});const t=document.getElementById("btn-start-info");t&&(t.onclick=()=>{i.ui.showGuide()});const s=document.getElementById("btn-start-settings");s&&(s.onclick=()=>{const l=window.game.audioManager.isMuted,o=[{label:l?"Bật Âm Thanh":"Tắt Âm Thanh",value:"toggle-mute",icon:l?"ph-speaker-high":"ph-speaker-slash"}];i.ui.promptOptions("Cài Đặt Hệ Thống",o,"Tùy chỉnh các thiết lập cơ bản của trò chơi.").then(async a=>{if(a==="toggle-mute"){const d=await window.game.audioManager.toggleMute();window.game.updateMuteIcon(d),i.ui.toast(d?"Đã tắt âm thanh":"Đã bật âm thanh","info")}})},window.game&&window.game.audioManager&&window.game.updateMuteIcon(window.game.audioManager.isMuted));const c=document.getElementById("btn-start-discord");c&&(c.onclick=()=>{i.ui.toast("Tính năng cộng đồng đang được phát triển.","info")}),this.generateParticles()}generateParticles(){const n=this.container.querySelector(".sword-qi-particles");if(n)for(let e=0;e<15;e++){const t=document.createElement("div");t.className="sword-qi-particle",t.style.left=`${Math.random()*100}%`,t.style.animationDelay=`${Math.random()*5}s`,t.style.animationDuration=`${3+Math.random()*4}s`,n.appendChild(t)}}}export{w as StartScreen};
